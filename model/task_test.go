...
func cleanupTaskTest() {
	_ = mongo.GetMongoCol(TaskColName).Delete(nil)
...
func TestTask_Add(t *testing.T) {
	...
	col := mongo.GetMongoCol(TaskColName)
	col.GetContext()
...
func TestTask_Save(t *testing.T) {
	...
	err = mongo.GetMongoCol(TaskColName).FindId(task.Id).One(&task)
	require.Nil(t, err)
	require.Equal(t, spider.Id, task.SpiderId)

	err = mongo.GetMongoCol(SpiderColName).FindId(task.SpiderId).One(&spider)
	require.Nil(t, err)
	require.Equal(t, spider.Id, task.SpiderId)
